...
function App() {
  ...
  const terminalHeight = useStore.terminalHeight(
    (state) => state.terminalHeight,
  );
  const terminalTabs = useStore.terminalTabs((state) => state.terminalTabs);
  const activeTerminalTabId = useStore.activeTerminalTabId(
    (state) => state.activeTerminalTabId,
  );
  const activeBottomSection = useStore.activeBottomSection(
    (state) => state.activeBottomSection,
  );
  ...
  const togglePanel = useCallback(
    ({
      ...
      panel: 'file-explorer' | 'search' | 'git' | 'task-list' | 'langPackPanel';
    }) => {
  ...
  const closeTerminalTab = (id: string, e: React.MouseEvent) => {
    ...
    if (activeTerminalTabId === id) {
      const next =
        remaining.length > 0 ? remaining[remaining.length - 1].id : '';
  ...
  const saveFile = async (path: string, content: string): Promise<void> => {
    ...
    useStore.openTabs
      ...
      .setOpenTabs(
        currentTabs.map((t) =>
          t.path === path ? { ...t, content: formatted } : t,
        ),
      );
  ...
  useEffect(() => {
    const handler = (
      _: unknown,
      { filePath, output }: { filePath: string; output: string },
    ) => {
      const lines = output
        .split('\n')
        .map((l: string) => l.trim())
      ...
      if (filePath === activeTabPathNow) {
        ...
        if (lines.length > 0) {
          useStore.terminalOpen.getState().setTerminalOpen(true);
          useStore.activeBottomSection
            .getState()
            .setActiveBottomSection('problems');
        }
      }
    };
    window.ipcRenderer.on('diagnostics:result', handler);
    return () => {
      window.ipcRenderer.off('diagnostics:result', handler);
    };
  }, []);
  const quotes = [
  ...
  useEffect(() => {
    async function loadStyles() {
      ...
      if (!activeThemeId) return;
      const themeStyles =
        await window.ipcRenderer.getSpecificTheme(activeThemeId);
  ...
  const { onKeystroke, onSave } = useHackatime(
    activeTab?.path ?? null,
    activeTab?.name ?? null,
    folderPath ?? null,
  );
    ...
    <div className="relative h-screen w-screen overflow-hidden">
      ...
      {videoPath != null && (
        <video
          ...
          onError={() => {
            if (videoPath !== './Chillhop_White_Oak.mp4')
              setVideoPath('./Chillhop_White_Oak.mp4');
          }}
      ...
      <div className="h-screen w-screen   text-white overflow-hidden flex flex-col">
        ...
        <div className="flex flex-row flex-1 overflow-hidden">
          ...
          >
                  ...
                  <div
                    ...
                    style={
                      tab.path === activeTabPath
                        ? {
                            color:
                              editorStyles.tabsActiveTextColor || '#E8C088',
                            backgroundColor:
                              editorStyles.tabsActiveBackground || '#0F0B08',
                            borderTop: `1px solid ${editorStyles.tabsBorder || '#E8C088'}`,
                          }
                        : {
                            color:
                              editorStyles.tabsInactiveTextColor || '#6B5D4A',
                          }
                    }
                  >
            ...
            {terminalOpen && (
              <div
                className="flex flex-col flex-shrink-0 border-t-2 border-t-[#3D3020]"
                ...
                <div className="flex items-center border-b border-b-[#3D3020] bg-[#1A1208] flex-shrink-0">
                  <button
                    onClick={() =>
                      useStore.activeBottomSection
                        .getState()
                        .setActiveBottomSection('terminal')
                    }
                  ...
                  {activeBottomSection === 'terminal' &&
                    terminalTabs.map((tab) => (
                      <div
                        ...
                        style={{
                          display:
                            tab.id === activeTerminalTabId ? 'flex' : 'none',
        ...
        {fileMenuOpen && (
          <div
            ...
            onClick={() =>
              useStore.fileMenuOpen.getState().setFileMenuOpen(false)
            }
          />
        )}
      </div>
